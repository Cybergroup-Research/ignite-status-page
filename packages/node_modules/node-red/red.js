...
try { var bcrypt = require('bcrypt'); }
catch(e) { bcrypt = require('bcryptjs'); }
var nopt = require("nopt");
var path = require("path");
var fs = require("fs-extra");
var RED = require("./lib/red.js");
import MetadataService from '../@node-red/ignite/service/metadataService';
var createTable = require("../@node-red/ignite/database-operations/provisioning/createTable");


var server;
var app = express();

// middleware to add ignite header
app.use(function addigniteheader (req, res, next) {
    res.setHeader('x-node-engine', 'ignite');
  next()
})
...
process.env.PROJECT_ROOT_FOLDER_PATH = process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH;
process.env.IGNITE_HOME_PATH = path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH, ".node-red");
...
try {
    ...
    settings.isApplicationLoaded = false;
    process.env.IS_APPLICATION_LOADED = false; 
...
async function initialize() {
    try {
        ...
        await createTable.createIgniteMetadataTable();
        let result = await metadataService.restoreMetaDataFiles();
        await metadataService.watchConfigFile();
        await metadataService.watchPackageLockFile();
        await metadataService.watchPackageFile();
        await metadataService.watchLibFolder();
    ...
    RED.start().then(function () {
        if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
            ...
            server.listen(settings.uiPort, settings.uiHost, function () {
                ...
                settings.isApplicationLoaded = true;
                process.env.IS_APPLICATION_LOADED = true;
            });
        } else {
