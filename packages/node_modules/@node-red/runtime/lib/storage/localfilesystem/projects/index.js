...
import ProjectsService from '../../../../../ignite/service/projectsService';
...
var createTable = require("../../../../../ignite/database-operations/provisioning/createTable")
            ...
            gitTools.init(_settings).then(function(gitConfig) {
                if (!gitConfig || /^1\./.test(gitConfig.version)) {
                    ...
                    if (!settings.readOnly) {
                        let result = false;
                        (async () => {
                            let isTablePresent = await createTable.createProjectDetailsTable();
                            settings.isProjectTblPresent = isTablePresent;
                            console.log("Creating Projects folder");
                            if (isTablePresent) {
                                result = await projectService.restoreProjectsFolder();
                                if (result) {
                                    setActiveProjects();
                                }
                            }
...
function setActiveProjects() {
    fs.ensureDir(projectsDir)
        ...
        .then(function (globalSettings) {
            ...
            if (initialFlowLoadComplete && setFlowsFunction) {
                initialFlowLoadComplete = false;
                setFlowsFunction(null, null, 'load');
            }
...
//this function is under runtime/lib/nodes/flows/index
var setFlowsFunction;

function getFlows(setFlows) {
    if (!initialFlowLoadComplete) {
        initialFlowLoadComplete = true;

        if (setFlows) {
            setFlowsFunction = setFlows;
        }
...
function saveProjectDetailsInDB() {
    if (settings && settings.isApplicationLoaded && settings.isProjectTblPresent && settings.editorTheme.projects.enabled) {
        projectService.saveProjectsFolderInDB();
    }
}
